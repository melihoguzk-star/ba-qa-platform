...
def save_keys_to_secrets():
    ...
    try:
        ...
        return True, f"✅ Keys saved to {secrets_file}"
    except Exception as e:
...
with tab1:
    ...
    st.info("ℹ️ Keys are currently stored in session. Save them to persist across restarts.")
    
    # Generate TOML content for preview
    toml_preview = "# API Keys Configuration\n"
    toml_preview += "# Auto-generated by BA&QA Platform Settings\n\n"
    
    if st.session_state.gemini_keys:
        if len(st.session_state.gemini_keys) == 1:
            toml_preview += f'GEMINI_API_KEY = "{st.session_state.gemini_keys[0]}"\n'
        else:
            toml_preview += "GEMINI_API_KEYS = [\n"
            for key in st.session_state.gemini_keys:
                toml_preview += f'    "{key}",\n'
            toml_preview += "]\n"
    ...
    if st.session_state.get("anthropic_key"):
        toml_preview += f'ANTHROPIC_API_KEY = "{st.session_state.anthropic_key}"\n'
    ...
    with col1:
        if st.button("💾 Kaydet", use_container_width=True, type="primary"):
            ...
            if success:
                ...
                st.error(message)
                st.warning("💡 Use Download button instead")
    ...
    with col2:
        st.download_button(
            ...
            help="Download and place in .streamlit/ folder"
        )
    
    with col3:
        st.caption("Save directly or download manually")
    
    # Show preview
    with st.expander("👁️ Preview secrets.toml"):
        st.code(toml_preview, language="toml")
        ...
        with st.form("add_anthropic_key"):
            ...
            if st.form_submit_button("➕ Add Anthropic Key", use_container_width=True):
                if new_anthropic_key:
                    ...
                    st.toast("✅ Anthropic key added!", icon="✅")
