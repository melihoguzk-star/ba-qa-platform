...
with tab1:
    ...
    st.info("ℹ️ Keys are currently stored in session. Click 'Save' to persist them to secrets.toml file.")
    ...
    with col1:
        if st.button("💾 Kaydet", use_container_width=True, type="primary"):
            ...
            if success:
                st.toast("✅ API keys saved successfully!", icon="✅")
                st.success(message)
            else:
                st.toast("❌ Failed to save keys", icon="❌")
                st.error(message)
    
    with col2:
        st.caption("Saves all configured API keys to .streamlit/secrets.toml")
